...
st.title("🎯 سیستم جامع و هوشمند بازاریابی غرفه‌داران باسلام")
st.markdown("بدون ارور ۴۰۴ | انتخاب صنف، کپی خودکار پیام و ورود سریع به بازار باسلام")
st.markdown("---")
...
# دیتابیس پاک و امن غرفه‌های برتر بدون لینک‌های خطادار
@st.cache_data
def get_comprehensive_database():
    ...
    categories_dict = {
        "🪵 صنایع دستی و چوبی": [
            ...
            "خراطی مدرن", "صنایع چوبی باران", "تندیس‌های چوبی", "دکور چوب شهرزاد", "معرق کده",
...
st.markdown("---")
st.subheader("💬 ۳. انتخاب غرفه، کپی پیام و ورود به باسلام")

selected_store = st.selectbox("از لیست بالا، غرفه مورد نظر خود را انتخاب کنید:", df_filtered["نام غرفه"].tolist())

if selected_store:
    # لینک جستجوی دقیق نام غرفه در باسلام که همیشه بدون ارور کار می‌کند
    search_url = f"https://basalam.com/search?q={urllib.parse.quote(selected_store)}"
...
موفق و پرفروش باشید"""
    ...
    with btn_col1:
        st.markdown(f'<a href="{search_url}" target="_blank"><button style="width:100%;background-color:#FF5A5F;color:white;padding:14px;border:none;border-radius:6px;font-size:16px;font-weight:bold;cursor:pointer;">🔍 جستجو و ورود به غرفه "{selected_store}"</button></a>', unsafe_allow_html=True)
